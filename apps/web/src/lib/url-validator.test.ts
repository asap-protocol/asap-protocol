...
describe('isAllowedExternalUrl', () => {
  ...
  it('allows valid external URLs when DNS resolves to public IP', async () => {
    ...
    expect(await isAllowedExternalUrl('https://example.com/manifest')).toMatchObject({
      valid: true,
      ips: expect.any(Array),
    });
    expect(await isAllowedExternalUrl('https://api.myagent.io')).toMatchObject({
      valid: true,
      ips: expect.any(Array),
    });
  });
  ...
  it('allows http scheme when DNS resolves to public IP', async () => {
    resolve4Spy.mockResolvedValue(['93.184.216.34']);
    expect(await isAllowedExternalUrl('http://example.com')).toMatchObject({
      valid: true,
      ips: expect.any(Array),
    });
  });
  ...
  it('blocks cloud metadata IPs', async () => {
    ...
    expect((await isAllowedExternalUrl('http://169.254.169.254/latest/meta-data')).valid).toBe(
      false
    );
  });
