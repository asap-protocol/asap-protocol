...
- **Reactivate approval bypass** — ``reactivate_agent`` /
  ``POST /asap/agent/reactivate`` refuse ``pending`` and ``rejected``
  sessions, not only ``revoked``. LIFE-004 scopes reactivation to expired
  agents; force-activating a denied or still-open registration would skip
  host consent.
