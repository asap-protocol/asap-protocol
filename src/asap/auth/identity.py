...
class AgentStore(Protocol):
    ...
    async def save(self, agent: AgentSession) -> None:
        ...
        Must not replace a ``revoked`` row with a non-revoked snapshot: a stale
        get→mutate→full-row save can otherwise undo ``revoke``.
        """
        ...
...
async def save_agent_unless_revoked(agent_store: AgentStore, agent: AgentSession) -> None:
    ...
    Example:
        ...
        ...     store, session.model_copy(update={"public_key": new_jwk})
        ... )
    """
